...
function validate_dni_ruc(run) {
    /* alert(run); */

    if (run === '9999999999999') {
        return true;
    }

    var expr = run.substr(0, 2);
    if (expr === "EX" | expr === "ex") {
        return true;
    }
    ...
    var ok = 1;
    for (i = 0; i < run.length && ok == 1; i++) {
        var n = parseInt(run.charAt(i));
        if (isNaN(n)) ok = 0;
    }
    if (ok == 0) {
    ...
    provincia = run.substr(0, 2);
    if (provincia < 1 || provincia > dniProvincias) {
        //alert('El código de la provincia (dos primeros dígitos) es inválido');
        return false;
    }

    /* Aqui almacenamos los digitos de la run en variables. */
    d1 = run.substr(0, 1);
    d2 = run.substr(1, 1);
    d3 = run.substr(2, 1);
    d4 = run.substr(3, 1);
    ...
    if (pub == true) {
        if (digitoVerificador != d9) {
            //alert('El rut de la empresa del sector público es incorrecto.');
            return false;
        }
        /* El rut de las empresas del sector publico terminan con 0001*/
        if (run.substr(9, 4) != '0001') {
            //alert('El rut de la empresa del sector público debe terminar con 0001');
            return false;
        }
    } else if (pri == true) {
        if (digitoVerificador != d10) {
            //alert('El rut de la empresa del sector privado es incorrecto.');
            return false;
        }
        if (run.substr(10, 3) != '001') {
            //alert('El rut de la empresa del sector privado debe terminar con 001');
            return false;
        }
    } else if (nat == true) {
        if (digitoVerificador != d10) {
        ...
        if (run.length > 10 && run.substr(10, 3) != '001') {
            //alert('El rut de la persona natural debe terminar con 001');
            return false;
        }
    }
    return true;
...
function dialog_delete_with_ajax(title, content, url, parameters, callback) {
    $.confirm({
        theme: 'material',
        title: title,
        icon: 'fas fa-trash',
